...
def test_a_superseded_budget_request_via_text_command_reports_superseded(operator_bot):
    ...
    assert any(
        "already processed or superseded" in m["text"] for m in operator_bot.client.sent_messages
    )
    assert not any(
        "out of range or invalid" in m["text"] for m in operator_bot.client.sent_messages
    )
...
def _request_cards(operator_bot, request_id: str) -> list[dict[str, Any]]:
    return [
        ...
        if request_id in str(message.get("text") or "") and message.get("reply_markup") is not None
    ]
...
def test_legacy_failed_single_owner_regression(operator_bot, monkeypatch):
    ...
    # 3. Seed request-scoped legacy evidence with card_intent_event followed by
    # card_failure_event under budget-request:req-b
    ...
    intents_before = len(
        [
            e
            for e in store.list_system_events_by_type("telegram_ui_card", limit=None)
    ...
    # Fake client has exactly one successful send_message for B and B card is buttonless
    # while predecessor A is incomplete
    assert len(operator_bot.client.sent_messages) == 1
    sent_msg = operator_bot.client.sent_messages[0]
    assert sent_msg["chat_id"] == 1
    assert (
        sent_msg.get("reply_markup") is None
        or sent_msg["reply_markup"].get("inline_keyboard") == []
    )
    ...
    handled_resume = operator_bot.process_update(callback_update("operator:wfresume:funding:req-a"))
    ...
    # 8. Assert activated finally refresh edits the same workflow-scoped physical message
    # into the live actionable B representation
    ...
    edited_for_wf = [
        m
        for m in operator_bot.client.edited_messages
        if m.get("reply_markup")
        and any(
            "req-b" in str(b.get("callback_data"))
...
def test_valid_head_with_incomplete_funding_predecessor_refuses_budget_confirm_and_cancel_callbacks(
    ...
    handled = operator_bot.process_update(callback_update("operator:budget:sel:req-b:r"))
    ...
    handled_cancel = operator_bot.process_update(callback_update("operator:budget:cancel:req-b"))
...
def test_budget_admission_allowed_once_funding_predecessor_completed(operator_bot, monkeypatch):
