...
def _stub_child_signal(router, monkeypatch, signal_run_id: str = "signal_child") -> None:
    """Stand in for the child run a budget selection now always requires.

    The selection is an input to the transition, not its terminal state, so
    _confirm_budget_request refuses outright without a signal config and
    completes only after the child run exists. These callback-parsing tests
    care about neither, so the child run is stubbed rather than executed.
    """
    monkeypatch.setattr(
        router,
        "_run_child_signal",
...
def test_telegram_operator_budget_callback_records_selected_budget(tmp_path, monkeypatch):
    config = load_config(_telegram_config_path(tmp_path))
    store = StateStore(config.state.sqlite_path, config.portfolio.initial_cash)
    ...
    _stub_child_signal(router, monkeypatch)
...
def test_telegram_operator_budget_callback_accepts_legacy_select_format(tmp_path, monkeypatch):
    config = load_config(_telegram_config_path(tmp_path))
    store = StateStore(config.state.sqlite_path, config.portfolio.initial_cash)
