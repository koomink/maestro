...
- 값: `run_id`, `approval_id`, `acquired_at`, `expires_at`
- 획득: 만료되지 않은 같은 키의 리스가 없을 때만. 짧은 `writer_lock` 안의 단일 트랜잭션
- 해제: 집행 완료 또는 중단 시
- 만료: 폴링 예산(단계당 20회 × 30초) × 단계 수 + 여유
...
flock은 프로세스가 죽으면 커널이 놓아준다. 리스는 놓아주지 않는다. 집행 중 프로세스가 죽으면 **만료까지 그 계정의 다음 승인이 막힌다.**

이는 받아들일 만하다. 잠금이었다면 같은 상황에서 다음 승인이 *교착*했고(8/11·8/12가 그것이다), 리스는 *명시적 거부*를 만든다 — 만료 시각과 소유자를 지목하는 거부다. 그리고 죽은 집행이 남긴 상태는 어차피 복구가 필요하다.

만료된 리스를 안전하게 회수하는 세대 펜싱은 **2부**의 몫이다. 이 스펙은 만료 시각을 기록하고 만료된 리스를 획득 가능하게 하는 최소 형태까지만 만든다.
...
- **`save_system_events_atomic`을 리스 획득에 사용** — 리스에는 만료 시각이 필요한데 `duplicate_key` CAS로는 만료를 표현할 수 없다. `UNIQUE(account_id, currency)` + `expires_at`을 가진 전용 테이블이 맞는 도구다. 3a-2 API의 제자리는 여전히 2부다.
- **만료 리스의 세대 펜싱** — 만료 시각 기록과 만료된 리스의 획득 가능성까지만 만든다. 죽은 집행이 남긴 상태를 안전하게 회수하는 것은 2부다.
...
- 신규 회귀 테스트:
  ...
  - **만료된 리스는 획득 가능하다** — 만료 시각을 과거로 둔 리스가 다음 집행을 영구히 막지 않는다
