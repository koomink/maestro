...
시장별 리밸런스 건(예: 트랑필로 KR, 크레센도 US)마다 하나 생성,
`signal_run_id` 단위로 추적. 같은 메시지가 단계를 따라 edit된다.

```
신호 생성 ── 노옵 ──→ "오늘은 매매할 것이 없어요" 한 줄 알림 후 종료
    │
    └─ 액션 필요 ──→ ⏳ 승인 대기 (승인/거절/자세히 버튼)
...
1. 카드 전송 시 `telegram_ui_card` system event로
   ...
   `card_key`는 카드 유형별 원천 ID로 정한다: 데일리 카드 = `daily:<signal_run_id>`,
   월간 자금 카드 = `funding:<request_id>`, 예외 카드 = `<유형>:<원천 event id>`.
...
- **UI 렌더링 실패는 거래 로직을 막지 않는다**: 카드 갱신 실패는 audit 이벤트만
  남기고 다음 sweep에서 재시도. 승인/주문 처리는 UI와 독립적으로 동작.
...
| 2 | 라이프사이클 카드 매니저 + 데일리 카드 + 노옵 한 줄 + 주문 알림 스팸 제거 | 구조 개편의 핵심 |
| 3 | 월간 자금 카드 (입금·예산 통합) | 월초 경험 개선 |
| 4 | 예외 마법사 (재주문·cash drift·복구·안전 정지) | 장애 대응 경험 개선 |
| 5 | 조회 카드 5종 + 구 명령어 메뉴 숨김 | 마무리 |
