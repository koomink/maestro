...
**카드의 수신자는 최초 전송 때 정해진다.** `refresh`가 매번 현재 설정을 읽으면,
허용 채팅을 하나 추가한 순간 과거의 모든 카드가 "복사본이 없다"로 보여 신규
전송된다 — 없는 복사본과 처음 보내는 카드는 구분되지 않는다. 그래서
`CardLifecycleManager.audience()`가 기준이다: 복사본이 하나도 없으면 현재 설정된
채팅 전부(아직 태어나지 않은 카드), 있으면 이미 복사본을 가진 채팅 중 지금도
설정에 있는 것들. 종결 판정(`_card_is_settled`)과 렌더 실패 기록도 같은 기준을 쓴다.
