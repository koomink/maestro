...
### 배치 자기 키 검사가 전제조건보다 먼저다

재실행(replay) 판정이 전제조건 판정보다 앞서야 한다. 예: head v2를 쓰는
배치가 `forbid=["head:wf:v2"]`를 걸었다면, 이미 커밋된 배치의 재실행은
전제조건에서 먼저 걸려 "경쟁에 졌다"로 오분류된다. 자기 키가 전부 존재하면
그건 **내가 이미 커밋한 것**이므로 멱등 성공으로 되돌려준다.
